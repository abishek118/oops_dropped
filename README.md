...
* 🏃‍♂️ **Background Fall Detection**: Runs silently in the background monitoring accelerometer magnitude to detect free falls.
* 💡 **Visual Alerts**: Instantly blinks the camera flashlight and maximizes screen brightness to draw attention when a fall is detected.
* 🔊 **Audible Alarm**: Plays a loop siren warning sound when triggered.
* 🔒 **Secure Alarm Stop**: Toggling "Secure Stop" requires the user to enter their lock screen passcode (PIN, pattern, password, or biometrics) to silence the alarm.
* 🎨 **Adaptive Design**: Features a premium circular logo indicating service state (full-color glow when active, grayscale when disabled).
* 📜 **Scrollable Dashboard**: Beautifully handles compact screen dimensions without layout errors.
